...
def add_message(conversation_id: str, role: str, content: str, status: str, query_plan: dict | None = None, facts: dict | None = None) -> AssistantMessage:
    ...
    try:
        ...
        db.commit()
        return AssistantMessage(message_id=message_id, role=role, content=content, status=status, facts=facts, created_at=created_at)
    finally:
        db.close()
...
def get_messages(conversation_id: str) -> list[AssistantMessage]:
    ...
    try:
        ...
        result = []
        for row in rows:
            result.append(AssistantMessage(message_id=row["message_id"], role=row["role"], content=row["content"], status=row["status"], facts=json.loads(row["facts_json"]) if row["facts_json"] else None, created_at=row["created_at"]))
        return result
    finally:
        db.close()
...
def last_query_plan(conversation_id: str) -> dict | None:
    db = connect()
    try:
        row = db.execute("SELECT query_plan_json FROM ai_messages WHERE conversation_id = ? AND query_plan_json IS NOT NULL ORDER BY created_at DESC LIMIT 1", (conversation_id,)).fetchone()
